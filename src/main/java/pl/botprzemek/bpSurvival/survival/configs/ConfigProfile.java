...
public class ConfigProfile extends Config {
    ...
    public Profile getProfile(UUID playerUUID) {
        ...
        if (config == null) return new Profile(new HashMap<>(), new HashMap<>(), new HashMap<>());
        ...
        HashMap<UUID, Boolean> letters = new HashMap<>();
        for (String letter : config.getStringList("letters")) letters.put(UUID.fromString(letter), true);

        return new Profile(homes, cooldowns, letters);
    }
    ...
    public void setProfile(UUID playerUUID, Profile profile) {
        ...
        for (String homeName : profile.getHomes().keySet()) {
            ...
            set(path + ".homes." + homeName + ".world", Objects.requireNonNull(location.getWorld()).getName());
            set(path + ".homes." + homeName + ".x", location.getX());
            set(path + ".homes." + homeName + ".y", location.getY());
            set(path + ".homes." + homeName + ".z", location.getZ());
        }

        for (String cooldownName : profile.getCooldowns().keySet()) {
            set(path + ".cooldowns." + cooldownName, profile.getCooldowns().get(cooldownName));
        }
    }
}
